...
- `server/messaging.ts` — outbound SMS templates fire as side effects of transitions; inbound goes through `extractJson()` (`server/llm.ts`, Claude with `output_config.format` JSON schema) with vendor's open orders as context. **Confidence gate: ≥ 0.8 with a resolved order auto-applies; otherwise → review queue** (`review_status = 'needs_review'`). This gate is the project's AI-safety story — don't bypass it. A digit never reaches it: `handleVendorInbound()` resolves an owned digit through the route table first, and a digit nothing owns is asked about rather than guessed at.
- **`server/slots.ts` — rotating reply codes.** SMS is one flat thread with no reply-to, so a vendor with several open questions can only really answer the newest and the rest are buried. Each open question owns a digit **pair** — (1,2) (3,4) (5,6) (7,8) (9,0), odd = affirmative — and states its own pair in its body, so nothing has to be remembered or answered in order. Five deep; the sixth question is refused and one rate-limited `v_backlog_digest` goes out instead. **A templated vendor message always owns a pair** — send questions through `sendVendorQuestion()`, never `sendToVendor()` (which is now conversational sends only). A follow-up about an order already asked about reuses that order's pair. The pure digit maths is in `shared/slots.ts` because the phone simulator labels bubbles with it.
...
- Client: 13 routes in one React app across hospice, vendor and field surfaces — see `docs/UX-FLOWS.md` for the page map and who reaches what. `App.tsx` splits them three ways: `/caregiver` and `/vendor-phone` render with **no chrome at all** (full-screen phone simulators), the three token routes a vendor can be texted (`/o/:token`, `/portal/:token`, `/status/:token`) render in `PortalShell` (brand + live dot, never our nav), and everything else renders inside the Shell with the global nav. Roles live in `client/src/lib/auth.tsx` (`ROLES`, `useAuth()`, localStorage-backed) and filter the nav bar only — **routes are deliberately not guarded**, so every screen stays reachable by URL mid-demo. Pages fetch with `useLive()` (`client/src/lib/useLive.ts`), which refetches on every SSE event — no manual cache invalidation; after a POST the SSE broadcast triggers the refresh.
...
`docs/PROBLEM-THESIS.md` is the why behind the design (the reporting-cost framing, the vendor adoption ladder, the integration asymmetry) — read it before changing scope or pitch content. `docs/BUILD-DAY-TASKS.md` is the shared task list (claim items by name). **Building frontend/UI? Read `docs/DESIGN-SYSTEM.md` first** — it's the visual source of truth (BetterRX coral+navy tokens, the shadcn/ui setup + atoms/mocks/hooks the Foundation lane landed, and two must-know gotchas: **don't re-run `shadcn init`** on Tailwind v4, and migrate the legacy `client/src/components/ui.tsx` per-page). `docs/FRONTEND-TASKS.md` is the frontend ticket list; `docs/design/*.html` are browsable screen references. `docs/BACKEND-SPEC.md` is the architecture spec. `docs/deliverables/` holds the submission drafts (AI approach, differentiation, integration sketch, demo script, assumptions register, slides) — keep them in sync with reality as the build evolves. **Added, deleted, or re-pointed a page? Update `docs/UX-FLOWS.md` in the same commit** — it's the page map and per-role navigation graph, and it's what answers a judge's "who logs in and what do they see?". Same rule when you change where a *user group* can go: a new nav link, a role filter on `surfaceLinks`, a route guard, a redirect, or any new `navigate()` call. Two things make this doc rot silently, so both are non-optional: (1) **derive the arrows, don't recall them** — re-run `grep -rnE "useNavigate|navigate\(|window\.location|<Link|<NavLink|href=" client/src/pages/ client/src/components/`, which is the whole truth about in-app navigation (everything else is the global nav bar); (2) **render the diagrams before pushing** — `npx -y @mermaid-js/mermaid-cli@9 -i docs/UX-FLOWS.md -o /tmp/mmd-check.md` — because broken mermaid renders as a silent error box in the reader's viewer, not a failed build, and balanced fences prove nothing. Keep to the conservative syntax already in the file (`graph`, plain subgraph titles, no emoji) so it renders in old viewers too. **Shipped a feature, wired something up, or found that something isn't wired up? Update `docs/FEATURES.md`** — it's the inventory read out of the *code* rather than out of the other docs, and it's the backbone of an honest pitch: sections 2 (*built but not surfaced*) and 3 (*designed only*) earn as many points as section 1, because FAQ §6 rewards saying what's real and penalises manufactured precision. It carries its own re-verify block (every endpoint, every screen, `npm run typecheck && npm test`) — **run it instead of editing from memory**, correct the test count, and update the "Last verified" line so the next person knows how much to trust the file. Same trap as the flow doc: the failure mode isn't a wrong sentence, it's a confident inventory nobody re-derived. **Built something worth showing off? Add a one-liner to the "Show-off inbox" in `docs/deliverables/SLIDES.md`** — it gets harvested into the pitch before code freeze; if it's not in the inbox, it won't be on stage.
...
2. **Post-death pickup**: seed scenario2 → EMR simulator (on `/demo` since the v8 board rebuild) marks patient deceased → pickup auto-appears on driver view → picked up with POD → family notified.
