...
- Client: 11 routes in one React app across hospice, vendor and field surfaces — see `docs/UX-FLOWS.md` for the page map and who reaches what. `App.tsx` splits them: `/caregiver` and `/vendor-phone` render **outside** the Shell (full-screen phone simulators, no nav bar), everything else renders inside it and gets the global nav. Roles live in `client/src/lib/auth.tsx` (`ROLES`, `useAuth()`, localStorage-backed) and filter the nav bar only — **routes are deliberately not guarded**, so every screen stays reachable by URL mid-demo. Pages fetch with `useLive()` (`client/src/lib/useLive.ts`), which refetches on every SSE event — no manual cache invalidation; after a POST the SSE broadcast triggers the refresh.
