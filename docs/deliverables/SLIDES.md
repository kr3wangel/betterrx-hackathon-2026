...
- The board answers "can this patient go home?" by name: "Ready for discharge — 2 of 2 confirmed" / "NOT ready — 1 of 2 unconfirmed", derived per patient from order states (confirmed = the vendor has committed; pickups excluded) — grouped row + row detail on `/board` (`dischargeReadiness()` in `client/src/lib/board.ts`) — slide 2 (closes bounty req H2 with the sponsor's own words)
- **The dispatcher's day, and their word on capacity:** the no-login portal now opens with a "Today" strip — load counted in *stops* (their unit of work, from our ledger) and one tap to declare "how many stops can you take today?" That number shows up in the case manager's swap dialog next to the vendor's track record: *"3 stops open · says they can take 2 more today"* (or "no capacity signal" — Beehive never declares, in character). Declared ≠ verified; informs the choice, never gates it — `/portal/<token>` + SwapVendorDialog — slide 2 (bounty V1: fleet capacity without fleet theater)
- An admission is one order now: the nurse bundles bed + oxygen + walker on `/order` ("+ Add another item"), the vendor gets ONE text naming the manifest and the tightest deadline, and one digit accepts the whole bundle (a decline escalates every item, nothing silently dropped) — same manifest machinery as trip batching, pointed at deliveries — `server/orders.ts` + `/order` — slide 2/4 ("we batch the asking, never the answering" now covers both directions)
- Consumables reorder themselves on the payer's clock: CPAP mask/filters and wound dressings carry their Medicare replacement schedule, and once a delivered consumable's window elapses the watchdog places the next order — same state machine, vendor texted, ledger stamped "auto-resupply · payer cadence, no model," and never for a deceased patient — `server/resupply.ts` + the consumable note on `/order` — closes bounty row V5 (was our one clean MISS); Q&A ammo more than a demo beat, since the cadence is 30–90 days
- "Rung zero" is now literal: type a vendor's name + phone under the vendor picker on `/order` and place the order — the vendor exists, is invited (the order text carries their magic link), and is activated the moment they tap it or text back a digit. The presenter line "the hospice types a phone number in from its own rolodex" is a live click, not a metaphor — `/order` + `POST /api/vendors` — slide 3 (integration asymmetry) / scenario 3 if we want to type it on stage instead of using seeded Timpanogos
...
- **The whole product speaks one visual language.** A 6px status spine on every order card — red = will miss a deadline, navy = in motion, green = done with proof — so state reads pre-attentively (color before words), and a coral "Needs you" focal panel gives the case manager's one urgent decision a home the eye lands on first. Same spine + coral-hero grammar now carries across the board, the `/driver` job cards, and the `/portal` + `/status` vendor doors — one design system, not per-screen styling, and it makes the UI *simpler* for non-technical staff, not busier — `client/src/components/board/BoardRow.tsx` + `SPINE_CLASS` in `client/src/lib/domain.ts` — **slide 1 (the "this is a real, finished product" first impression)**
- 
- 
- 
