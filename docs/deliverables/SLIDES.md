...
- **The trust gap: what the vendor said vs what the ledger can prove.** Every delivery is either POD-verified (driver photo/signature) or the vendor's word — and Beehive's word runs ~18 points ahead of their PODs, because late unverified runs get called in as on-time. The gap is withheld below 10 deliveries per cohort (a gap on a handful of orders is noise, not a finding), plus interventions-per-order (chases + escalations = staff time each vendor costs). Computed live off the append-only ledger, never the seeded stats — it's a **contract-renewal argument, not an impression** — "Contract leverage" panel on `/reports` (`vendorLeverage()` in `server/reports.ts`) — the oversight slide, and the Q&A answer to "a dispatcher presses 1 to get you off the phone"
- 
- 
- 
- 
