...
- One thread, one digit, two meanings: reply "1" confirms the delivery arrived, and the very next "1" in the same thread flags the equipment unusable — disambiguated by the message template, never by AI guessing (`server/sms.ts` REPLY_ROUTES) — slide 4 (AI honesty) — ⚠️ **the argument is sound and the table is real, but the *family* half of it is not clickable:** `/caregiver` posts to `/api/orders/:id/condition-reply` (`condition.ts`), not to the REPLY_ROUTES table. Pitch it as the design, or wire `/caregiver` the way `/vendor-phone` now is. Don't demo it as a live thread.
- **Two trust levels in one thread, now clickable:** on `/vendor-phone` a dispatcher can *tap* a quick reply — resolved by the `template × digit` route table with **no model, no confidence score, nothing to review** — or *type* prose, which goes to Claude behind the 0.8 gate with the human queue underneath. Same thread, same ledger, and the UI even labels them differently ("sending…" vs "reading…"). `client/src/pages/VendorPhone.tsx` + `server/sms.ts` — **slide 4 (AI ROI): we spend the model only where the ambiguity is real, and here you can watch us not spend it**
- 
