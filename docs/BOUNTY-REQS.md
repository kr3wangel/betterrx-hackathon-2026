...
| # | Requirement (verbatim) | Status | What exists / what's missing |
|---|---|---|---|
| V1 | Fleet and route capacity, service area, and current load | **IN PROGRESS** (08-15, pre-freeze) | Spec'd (`docs/FLEET-CAPACITY-SPEC.md`, independently reviewed, 4 blockers fixed pre-build). **Merged already:** `vendor_capacity` table, `vendorLoad()` (stops = household × direction, due-today math, `remaining_today` computed server-side only), `POST /api/portal/:token/capacity`, `GET /api/vendors/load`, seeded declarations (Beehive deliberately never declares), narration mute — 23 tests. **Landing now:** the portal "Today" strip (load in stops + one-tap capacity declaration, no login) and the swap dialog's load + service-area lines at the moment of vendor choice. Final status will be **PARTIAL** by design: capacity is one declared number per day — no truck model, no routing, on purpose (the review barred a BUILT claim and we agree). |
