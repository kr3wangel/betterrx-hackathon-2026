...
|---|---|---|---|
| H1 | Patient and equipment need (type, quantity, urgency, target date) | **BUILT** | `/order`: patient + equipment (CMS-grounded catalog, now 15 codes incl. consumables), quantity, urgency (routine/urgent/STAT), needed-by with SLA defaults per urgency (`server/sla.ts`). Vendor texted on placement — and a **multi-item admission bundle sends ONE text** (`v_order_group`, 08-15): one reply pair for the lot, one digit accepts everything, a decline escalates every item. Same manifest machinery as trip batching — we batch the asking, never the answering. |
