...
| # | Requirement (verbatim) | Status | What exists / what's missing |
|---|---|---|---|
| V1 | Fleet and route capacity, service area, and current load | **PARTIAL** | Service area: on the vendor record and used for area lines in texts. Current load: the portal lists every order the vendor still owes something on, with SLA clocks; trip batching speaks the stop (their unit of work). **Fleet/route capacity: nothing** — no truck model, no capacity math. Adjacent, honestly: trips are the semantic foundation capacity planning would sit on. |
