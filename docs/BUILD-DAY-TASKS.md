...
- [ ] **M · Cost-of-care widget** — DME spend next to med spend per patient on the hospice view. *Needs the pricing item.* Lives on the Director of Nursing (DON) reports view (below).
- [ ] **M · Director of Nursing (DON) reports view** — the third hospice persona (briefing: ordering nurse / case worker / Director of Nursing) has no surface yet. A `/reports` view: vendor scorecards straight from `vendor_stats` (on-time by equipment × weekday — the risk engine's raw material, zero new data), open escalations, pickup latency, cost-of-care, and a **"phone calls that never happened" counter** (auto-applied vendor updates + auto-triggered pickups) — the north-star metric on an actual screen. Label each hospice surface with its persona.
