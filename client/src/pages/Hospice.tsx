...
export default function Hospice() {
  ...
  const board = useMemo(
    () => buildBoard(orders ?? [], escalations ?? [], nameOf),
    [orders, escalations, nameOf]
  )
  ...
  return (
    <div className="mx-auto max-w-[860px] pb-9">
      ...
      {board.needsYou.map((row) => (
        <BoardRow key={row.key} row={row} vendors={vendors ?? []} lead />
      ))}
      {queue.length > 0 && (
        <button
      ...
      {board.onTheWay.map((row) => (
        <BoardRow key={row.key} row={row} vendors={vendors ?? []} />
      ))}
      ...
      <div className="rounded-[14px] bg-card px-5 py-3.5 shadow-[0_1px_3px_rgba(38,50,64,.05)]">
        <div className="flex flex-wrap items-center justify-between gap-3 text-[14px] text-muted-foreground">
          <span>
            {board.done.withPod === board.done.completions
              ? 'Every delivery had a photo and a signature.'
              : `${board.done.completions - board.done.withPod} are still waiting on a photo.`}
          </span>
      ...
      <ReviewQueueDialog queue={queue} orders={orders ?? []} open={showReview} onOpenChange={setShowReview} />
    </div>
  )
}
