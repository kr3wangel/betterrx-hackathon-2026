...
export const CALLS_AVOIDED_DEFINITION =
  'Vendor status updates this system received without a human picking up a phone: inbound vendor texts the AI parsed confidently enough to auto-apply, updates the vendor recorded themselves (every vendor-actor event in the ledger, including magic-link portal taps), and pickups triggered by a nurse tap or the EMR webhook. Messages routed to human review are not counted. Computed from the event ledger on every request, never stored.'
...
export function reportSummary(): ReportSummary {
  const breakdown = {
    auto_applied_messages: count(
      "SELECT COUNT(*) AS n FROM messages WHERE direction = 'in' AND review_status = 'auto_applied'",
    ),
  ...
  return {
    calls_avoided:
      breakdown.auto_applied_messages + breakdown.vendor_self_service_updates + breakdown.auto_triggered_pickups,
